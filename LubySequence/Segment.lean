...
-/
@[simp]
public theorem segment0 : segment 0 = 1 := by simp [segment]

/--
The segment index at position 1 is 2.
-/
@[simp]
public theorem segment1 : segment 1 = 2 := by simp [segment]
...
-/
public theorem segment_is_pos : ∀ n : ℕ, segment n > 0 := by
  intro n
...
-/
public theorem segment_limit' (n : ℕ) : segment n ≤ n + 1 := by
  induction n using Nat.strong_induction_on with
  | h n ih =>
    rw [segment.eq_def]
    split
    ...
    · expose_names
      ...
      · expose_names
        ...
        · have n_ge_4 : n ≥ 4 := by
            ...
            · replace h : n ≥ 1 := one_le_iff_ne_zero.mpr h
              obtain h_1|h : n = 1 ∨ n > 1 := LE.le.eq_or_lt' h
              · exact absurd h_1 n_eq_1_or_3.left
              · replace h : n ≥ 2 := h
                obtain h_2|h : n = 2 ∨ n > 2 := LE.le.eq_or_lt' h
                · simp [h_2, size] at h_1
                · replace h : n ≥ 3 := h
                  obtain h_3|h : n = 3 ∨ n > 3 := LE.le.eq_or_lt' h
                  · exact absurd h_3 n_eq_1_or_3.right
                  · replace h : ¬n < 4 := Nat.not_lt.mpr h
...
-/
public theorem segment_limit {n : ℕ} (n_ge_2 : n ≥ 2) : segment n ≤ n := by
  induction n using Nat.strong_induction_on with
  | h n ih =>
    rw [segment.eq_def]
    split
    ...
    · expose_names
      ...
      · expose_names
        ...
        · refine Nat.add_le_add_iff_right.mpr ?_
          · exact Nat.le_self_pow (Nat.sub_ne_zero_iff_lt.mpr n2size_ge_3) 2
      · expose_names
        simp
        obtain eq|n_ge_3 : n = 2 ∨ n > 2 := by exact LE.le.eq_or_lt' n_ge_2
        · simp [eq, size] at h_1
        · replace n_ge_3 : n ≥ 3 := n_ge_3
          obtain eq|n_ge_4 : n = 3 ∨ n > 3 := by exact LE.le.eq_or_lt' n_ge_3
          · simp [eq, size]
          · replace n_ge_4 : n ≥ 4 := n_ge_4
            have n2size_eq_n1size : (n + 2).size = (n + 1).size := by
              ...
              · replace h_1 : ¬n + 2 = 2 ^ ((n + 2).size - 1) := by
                  by_contra x
                  have : n = 2 ^ ((n + 2).size - 1) - 2 := Nat.eq_sub_of_add_eq x
            ...
            · have n2size_eq_n1size : (n + 2).size = (n + 1).size := by
                ...
                have : (n + 2).size = (n + 1).size + 1 := by
                  obtain c|c : (n + 2).size = (n + 1).size ∨ (n + 2).size = (n + 1).size + 1 := by
              ...
              have n2size_eq_nsize : (n + 2).size = n.size := by
                have s2 : n.size = (n + 1).size := by
                  ...
                  obtain c|c : (n + 1).size = n.size ∨ (n + 1).size = n.size + 1 := size_limit n
              ...
              · simp [n2size_eq_nsize] at *
                ...
                · simp [←n_is_pow2] at *
                  have : n - (n - 1) = 1 := Nat.sub_sub_self (one_le_of_lt n_ge_4)
                  simp [this]
                  have s1 : 2 ^ (n.size - 2) + 2 ^ (n.size - 2) ≤ n := by
                    have t1 : 2 ^ (n.size - 1) ≤ n := n_ge_subenvelope (one_le_of_lt n_ge_4)
                ...
                · replace n'_eq_0 : n - (2 ^ (n.size - 1) - 1) > 0 := by
                      ...
                      have : n ≥ 2 + 2 ^ (n.size - 1) - 1 := by
                        have : n ≥ 2 ^ (n.size - 1) + 2 - 1 := by
                          have : n ≥ 2 ^ (n.size - 1) + 1 := by
                            have s1 : n ≥ 2 ^ (n.size - 1) := n_ge_subenvelope (one_le_of_lt n_ge_4)
                  ...
                  have : 2 ^ (n.size - 2) + (n - (2 ^ (n.size - 1) - 1)) ≤ n := by
                    ...
                    · refine Nat.sub_le_sub_left ?_ n
                      · exact le_sub_one_of_lt (Nat.lt_add_of_pos_right (Nat.two_pow_pos (n.size - 2)))
...
-/
public theorem segment_limit2 {n : ℕ} (n_ge : n ≥ 2) : segment n ≤ 2 ^ ((n + 1).size - 1) := by
  induction n using Nat.strong_induction_on with
  | h n ih =>
    have nsize_le_self : 2 ^ (n.size - 1) ≤ n := n_lower (zero_lt_of_lt n_ge)
    obtain n_eq_2|n_ge : n = 2 ∨ n > 2 := LE.le.eq_or_lt' n_ge
    · simp [n_eq_2, segment]
    · obtain n_eq_3|n_ge_4 : n = 3 ∨ n > 3 := LE.le.eq_or_lt' n_ge
        ...
        · have n1_ne_pow2 : ¬n + 1 = 2 ^ n.size := by
          ...
          · simp [n1size_eq_nsize, n2size_eq_nsize]
            ...
            · rename' segment_arg_eq_0 => segment_arg_pos
              ...
              obtain segment_arg_eq_1|segment_arg_ge_2 :
                          ...
                          · replace gt : n - 2 ^ (n.size - 1) + 1 + 1 ≤ 2 ^ (n.size - 1) := gt
                            ...
                            · have : n + 2 = 2 ^ n.size := by
                                ...
                                replace eq : n + 2 = 2 ^ (n.size - 1) + 2 ^ (n.size - 1) := by
                                  exact (Nat.sub_eq_iff_eq_add (le_add_right_of_le nsize_le_self)).mp eq
...
-/
public theorem segment_is_monotone : ∀ n : ℕ, segment n ≤ segment (n + 1) := by
  intro n
  induction n using Nat.strong_induction_on with
  | h n ih =>
    rw [segment.eq_def]
...
-/
public theorem segment_is_monotone' {a b : ℕ} (h : a ≤ b) : segment a ≤ segment b := by
  let d := b - a
...
-/
public theorem segment_prop1' : ∀ n : ℕ, n = 2 ^ n.size - 2 →
...
-/
public theorem segment_prop1 {n : ℕ} (h' : n = 2 ^ n.size - 2) : segment n = 2 ^ (n.size - 1) := by
    induction n using Nat.strong_induction_on with
    | h n ih =>
      by_cases n_eq_0 : n = 0
      · simp [n_eq_0]
      · have n_ge_2 : n ≥ 2 := by
          replace n_eq_0 : n ≥ 1 := one_le_iff_ne_zero.mpr n_eq_0
          obtain eq|gt : n = 1 ∨ n > 1 := LE.le.eq_or_lt' n_eq_0
          · simp [eq] at h'
          · exact gt
        by_cases n_eq_2 : n = 2
        · simp [n_eq_2, segment, size]
        · have n_ge_4 : n ≥ 4 := by
            have : n > 2 := Nat.lt_of_le_of_ne n_ge_2 fun a ↦ n_eq_2 (id (Eq.symm a))
            replace : n ≥ 3 := this
            by_cases n_eq_3 : n = 3
            · simp [n_eq_3] at h'
            · obtain c|c : n = 3 ∨ n > 3 := LE.le.eq_or_lt' this
            ...
            have : (n + 2).size = n.size + 1 := by
              replace h : n + 2 = 2 ^ ((n + 2).size - 1) := by
                ...
                have s2 : 2 ^ ((n + 2).size - 1) - 2 + 2 = 2 ^ ((n + 2).size - 1) := by
                  ...
                  · refine le_pow ?_
                    · refine zero_lt_sub_of_lt ?_
                      · have t1 : n + 2 ≥ 4 + 2 := Nat.add_le_add_right n_ge_4 2
                        replace t1 : (n + 2).size ≥ (4 + 2).size := size_le_size t1
...
-/
public theorem segment_prop2 : ∀ n > 0, ¬n = 2 ^ n.size - 2 →
...
public def segment_length (n : ℕ) : ℕ := trailing_zeros (segment n) + 1
...
@[simp]
public theorem segment_length_of_0_eq_1 : segment_length 0 = 1 := by simp [segment_length]

@[simp]
public theorem segment_length_of_1_eq_2 : segment_length 1 = 2 := by simp [segment_length]

@[simp]
public theorem segment_length_of_2_eq_2 : segment_length 2 = 2 := by simp [segment_length, segment]
...
-/
public theorem segment_length_prop1 : ∀ n > 0, n = 2 ^ n.size - 2 →
...
-/
public theorem segment_length_prop2 : ∀ n > 0, n = 2 ^ n.size - 2 →
...
-/
public theorem segment_length_prop3 : ∀ n > 0, ¬segment n = 2 ^ ((n + 1).size - 1) →
  ...
  | h n ih =>
    have n_ge_3 : n ≥ 3 := by
      obtain eq|n_gt_1 : n = 1 ∨ n > 1 := by exact LE.le.eq_or_lt' n_gt_0
      · simp [eq] at n_ne_envelope_segment
      · replace n_gt_1 : n ≥ 2 := by exact n_gt_1
        obtain eq|n_gt_2 : n = 2 ∨ n > 2 := by exact LE.le.eq_or_lt' n_gt_1
    ...
    have n_ne_envelope : ¬n = 2 ^ ((n + 2).size - 1) - 2 := by
      ...
      have n_is_envelope' : n + 2 = 2 ^ ((n + 2).size - 1) := by
        ...
        · exact le_pow (zero_lt_sub_of_lt (lt_of_add_left_lt n2size_gt_3))
      have n_gt_4 : n ≥ 4 := by
        obtain eq|gt : n = 3 ∨ n > 3 := by exact LE.le.eq_or_lt' n_ge_3
    ...
    · split <;> expose_names
      ...
      · simp
        have n_ne_envelope' : ¬n + 2 = 2 ^ ((n + 2).size - 1) := by
          have s1 : ¬n + 2 = 2 ^ ((n + 2).size - 1) - 2 + 2 := (add_ne_add_left 2).mpr n_ne_envelope
          have s2 : 2 ^ ((n + 2).size - 1) - 2 + 2 = 2 ^ ((n + 2).size - 1) := by
        ...
        · replace n_ge_4 : n ≥ 4 := n_ge_4
          have n_ge_7 : n ≥ 7 := by
            ...
            · replace gt : n ≥ 5 := gt
              obtain eq|gt : n = 5 ∨ n > 5 := LE.le.eq_or_lt' gt
              · simp [eq, segment, size] at n_ne_envelope_segment
              · replace gt : n ≥ 6 := gt
                obtain eq|gt : n = 6 ∨ n > 6 := LE.le.eq_or_lt' gt
                · simp [eq, segment, size] at n_ne_envelope_segment
                · replace gt : n ≥ 7 := gt
                  exact gt
          clear n_ge_4
          obtain n_eq_7|n_ge_8 : n = 7 ∨ n > 7 := LE.le.eq_or_lt' n_ge_7
          · simp [n_eq_7, size, trailing_zeros]
          · replace n_ge_8 : n ≥ 8 := n_ge_8
            have nsize_ge_4 : n.size ≥ 4 := by
              have s1 : n.size ≥ (8 : ℕ).size := size_le_size n_ge_8
            ...
            · have n1size_eq_nsize_add_1 : (n + 1).size = n.size + 1 := by
                ...
                have : (2 ^ ((n + 1).size - 1) - 1).size = (n + 1).size - 1 := by
                  ...
                  · refine zero_lt_sub_of_lt ?_
                    · refine one_lt_iff_ne_zero_and_ne_one.mpr ?_
                      · constructor
                        · exact Nat.ne_zero_iff_zero_lt.mpr (size_pos.mpr (Nat.add_pos_left n_gt_0 1))
              ...
              have n2size_eq_nsize_add_1 : (n + 2).size = n.size + 1 := by
                have : n + 1 + 1 = 2 ^ ((n + 1).size - 1) + 1 := congrFun (congrArg HAdd.hAdd at_seg_beg) 1
                replace : (n + 1 + 1).size = (2 ^ ((n + 1).size - 1) + 1).size := congrArg size this
                simp [n1size_eq_nsize_add_1] at this
                replace s1 : (2 ^ n.size + 1).size = n.size + 1 := by
                  exact size_add (Nat.one_pos) (Nat.one_lt_two_pow (Nat.ne_zero_of_lt nsize_ge_4))
                simp [s1] at this
                exact this
              ...
              simp [this]
            · have n1size_eq_nsize : (n + 1).size = n.size := by
                obtain eq|add1 : (n + 1).size = n.size ∨ (n + 1).size = n.size + 1 := size_limit n
                · exact eq
              ...
              have n2size_eq_nsize : (n + 2).size = n.size := by
                have : (n + 2).size = (n + 1).size := by
                  ...
                  have : n + 1 + 1 = 2 ^ (n + 1).size := by
                    ...
                    · obtain eq|gt : (n + 2).size = (n + 1).size ∨ (n + 2).size = (n + 1).size + 1 := by
                        exact size_limit (n + 1)
                      · exact absurd eq x
                      · exact gt
              ...
              · split at n_ne_envelope_segment <;> expose_names
                ...
                · simp at n_ne_envelope_segment
                  simp [n1size_eq_nsize, n2size_eq_nsize]
                  simp [n2size_eq_nsize] at n2size_gt_3
                  by_cases eq : 2 ^ (n.size - 1) = n
                  · simp [eq]
                    have : n - (n - 1) = 1 := Nat.sub_sub_self n_gt_0
                    simp [this]
                  ...
                  · rename' eq => n_ne_pow2
                    ...
                    have n_lt_pow2_minus_2 : n < 2 ^ n.size - 2 := by
                      ...
                      obtain eq|lt : n = 2 ^ n.size - 1 ∨ n < 2 ^ n.size - 1 := Nat.eq_or_lt_of_le this
                      ...
                      · replace lt : n ≤ 2 ^ n.size - 1 - 1 := le_sub_one_of_lt lt
                        replace lt : n ≤ 2 ^ n.size - 2 := by grind
                        obtain eq|lt : n = 2 ^ n.size - 2 ∨ n < 2 ^ n.size - 2 := Nat.eq_or_lt_of_le lt
                    ...
                    · have unsegment : segment (n / 2 - 1) ≤ 2 ^ (n.size - 2) := by
                        ...
                        have s4 : n / 2 - 1 + 1 = n / 2 := by
                          refine Nat.sub_add_cancel ?_
                          · exact (Nat.one_le_div_iff Nat.zero_lt_two).mpr (le_of_add_left_le n_ge_8)
                        simp [s4] at s1
                        replace s4 : (n / 2).size = n.size - 1 := by
                          ...
                          · rename' even => odd
                            ...
                            have left : ((n - 1) / 2).size = (n / 2).size := by
                              ...
                              have t1 : (n - 1) / 2 = (n - 1).div2 := Eq.symm (div2_val (n - 1))
                              simp [←t1] at n1_bits
                        ...
                        simp [this] at s1
                        exact s1
                      have segment_arg_order : n - (2 ^ (n.size - 1) - 1) ≤ n / 2 - 1 := by
                        have : n / 2 + 2 ≤ 2 ^ (n.size - 1) := by
                          have : n < 2 ^ n.size := by exact lt_size_self n
                          replace : n + 1 ≤ 2 ^ n.size := this
                          obtain eq|lt : n + 1 = 2 ^ n.size ∨ n + 1 < 2 ^ n.size := Nat.eq_or_lt_of_le this
                          · exact absurd eq n1_ne_pow2
                          · replace eq : n + 1 + 1 ≤ 2 ^ n.size := lt
                            simp [n_add1_add1] at eq
                            obtain eq|lt : n + 2 = 2 ^ n.size ∨ n + 2 < 2 ^ n.size := Nat.eq_or_lt_of_le lt
                            · exact absurd eq n2_ne_pow2
                            · replace lt : n + 3 ≤ 2 ^ n.size := by grind
                              obtain eq|lt : n + 3 = 2 ^ n.size ∨ n + 3 < 2 ^ n.size := Nat.eq_or_lt_of_le lt
                        ...
                        replace : n - (2 ^ (n.size - 1) - 1) ≤ n / 2 - 1 := by
                          have even : Even n := parity
                          replace even : n = n / 2 + n / 2 := by
                            rw [←mul_two]
                          ...
                          replace : n / 2 + 1 ≤ 2 ^ (n.size - 1) - 1 := by
                            ...
                            obtain eq|gt : n / 2 = 2 ^ (n.size - 1) - 1 ∨ n / 2 < 2 ^ (n.size - 1) - 1 := by
                              exact Nat.eq_or_lt_of_le this
                            · replace eq : n / 2 * 2 = (2 ^ (n.size - 1) - 1) * 2 := by
                                exact congrFun (congrArg HMul.hMul eq) 2
                              rw [mul_two, ←even, mul_comm] at eq
                          ...
                          replace : n - (2 ^ (n.size - 1) - 1) ≤ n - (n / 2 + 1) := by
                            exact Nat.sub_le_sub_left this n
                          replace : n - (2 ^ (n.size - 1) - 1) ≤ n / 2 - 1 := by
                            have t1 : n - (n / 2 + 1) = n / 2 - 1 := by
                              have : n - (n / 2 + 1) = n - n / 2 - 1 := rfl
                              rw (occs := .pos [3]) [even] at this
                              simp at this
                              exact this
                            simp [t1] at this
                            exact sub_le_of_le_add this
                      ...
                      have s1 : segment (n - (2 ^ (n.size - 1) - 1)) ≤ segment (n / 2 - 1) := by
                        exact segment_is_monotone' segment_arg_order
                    ...
                    · have odd : Odd n := by exact not_even_iff_odd.mp parity
                      ...
                      have odd'' : (n - 1) / 2 = n / 2 := by
                        refine Nat.div_eq_of_eq_mul_right ?_ ?_
                        · exact Nat.zero_lt_two
                        · exact Eq.symm (two_mul_odd_div_two (not_even_iff.mp parity))
                      have unsegment : segment (n / 2) ≤ 2 ^ (n.size - 2) := by
                        ...
                        replace s4 : (n / 2 + 1).size = n.size - 1 := by
                          ...
                          have aux : (n / 2 + 1).size = (n / 2).size := by
                            have : (n + 1) / 2 = (n + 2) / 2 := by
                              have : n + 2 = n + 1 + 1 := rfl
                              simp [this]
                              replace : (n + 1 + 1) / 2 = (n + 1) / 2 + 1 / 2 := by
                                refine add_div_of_dvd_add_add_one ?_
                                · refine (mod_eq_sub_iff ?_ ?_).mp ?_
                                  · exact Nat.one_pos
                                  · exact NeZero.one_le
                                  . simp
                                    have : Odd (n + 2) := by
                                      refine odd_add_one.mpr ?_
                                      · have : Even (n + 1) := Odd.add_one odd
                                        exact not_odd_iff_even.mpr this
                                    exact odd_iff.mp this
                              simp [this]
                            replace : (n + 1) / 2 = n / 2 + 1 := by
                              have aux : (n + 2) / 2 = n / 2 + 1 := by grind
                              simp [aux] at this
                              exact this
                            simp [←this]
                            replace : ((n + 1) / 2).size = (n + 1).size - 1 := by
                              refine size_div ?_ ?_
                              · exact le_add_right_of_le n_gt_0
                              · have : Even (n + 1) := by exact Odd.add_one odd
                                exact dvd_of_mod_eq_zero (even_iff.mp this)
                            simp [this, n1size_eq_nsize]
                            exact id (Eq.symm s)
                          simp [aux]
                          exact s
                        simp [s4] at s1
                        ...
                        exact s1
                      rw [add_comm]
                      have segment_arg_order : n - (2 ^ (n.size - 1) - 1) ≤ n / 2 := by
                        have : n / 2 + 2 ≤ 2 ^ (n.size - 1) := by
                          have : n < 2 ^ n.size := by exact lt_size_self n
                          replace : n + 1 ≤ 2 ^ n.size := this
                          obtain eq|lt : n + 1 = 2 ^ n.size ∨ n + 1 < 2 ^ n.size := Nat.eq_or_lt_of_le this
                        ...
                        replace : n - (2 ^ (n.size - 1) - 1) ≤ n / 2 := by
                          replace : n / 2 + 1 ≤ 2 ^ (n.size - 1) - 1 := by
                            ...
                            · replace eq : n / 2 * 2 = (2 ^ (n.size - 1) - 1) * 2 := by
                                exact congrFun (congrArg HMul.hMul eq) 2
                              have even : n /2 * 2 = n - 1 := by grind
                              rw [even] at eq
                              rw [Nat.sub_mul] at eq
                              replace eq : n + 1 = 2 ^ (n.size - 1) * 2 := by grind
                              rw [←pow_succ, nsize_minus1_add1] at eq
                              exact absurd eq n1_ne_pow2
                            · exact gt
                          replace : n - (2 ^ (n.size - 1) - 1) ≤ n - (n / 2 + 1) := by
                            exact Nat.sub_le_sub_left this n
                          replace : n - (2 ^ (n.size - 1) - 1) ≤ n / 2 := by
                            have t1 : n - (n / 2 + 1) = n / 2 := by rw (occs := .pos [1]) [odd'] ; simp
                            simp [t1] at this
                            exact sub_le_of_le_add this
                          exact this
                        exact this
                      have s1 : segment (n - (2 ^ (n.size - 1) - 1)) ≤ segment (n / 2) := by
                        exact segment_is_monotone' segment_arg_order
                      refine trailing_zeros_prop7 (n.size - 2) (segment (n - (2 ^ (n.size - 1) - 1))) ?_ ?_
                      · have : segment (n - (2 ^ (n.size - 1) - 1)) ≤ 2 ^ (n.size - 2) := by
                          exact Nat.le_trans s1 unsegment
                        obtain eq|lt :
                            segment (n - (2 ^ (n.size - 1) - 1)) = 2 ^ (n.size - 2) ∨
                            segment (n - (2 ^ (n.size - 1) - 1)) < 2 ^ (n.size - 2) := by
                          exact Nat.eq_or_lt_of_le this
                        · simp [n2size_eq_nsize, n1size_eq_nsize, eq, ←mul_two, ←pow_succ] at n_ne_envelope_segment
                          have aux : n.size - 2 + 1 = n.size - 1 := by grind
                          simp [aux] at n_ne_envelope_segment
                        · exact lt
                      · exact Nat.pos_iff_ne_zero.mp (segment_is_pos (n - (2 ^ (n.size - 1) - 1)))

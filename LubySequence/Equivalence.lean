...
-/
theorem segment_starts_shift (a : ℕ) (m : ℕ) (hm1 : 1 ≤ m) (hm2 : m ≤ 2 ^ a) :
...
theorem segment_starts_gt_self' (t : ℕ) : segment_starts (t + 2) > t :=
...
theorem envelope_S₂_eq (n : ℕ) (h : Luby.is_envelope n = true) :
    Luby.S₂ (n + 2) = n + 2 := by
  unfold Luby.is_envelope at h
  exact of_decide_eq_true h

/-- Local: from `is_envelope n`, derive `n + 2 = 2^j` where `j = (n+3).size - 1`. -/
theorem envelope_gives_pow2 (n : ℕ) (h : Luby.is_envelope n = true) :
    n + 2 = 2 ^ ((n + 3).size - 1) := by
...
theorem segment_starts_pow2_add_one (a : ℕ) : segment_starts (2 ^ a + 1) = 2 ^ (a + 1) - 1 := by
...
-/
theorem luby_via_segment_at_envelope (n : ℕ) (h : Luby.is_envelope n = true) :
...
-/
theorem luby_via_segment_non_envelope (n : ℕ) (h : ¬(Luby.is_envelope n = true)) :

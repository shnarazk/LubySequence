  ...
  case h n hn =>
    ...
    · simp [Luby.S₂]
      ...
      · -- to show a conflict
        ...
        simp [LubyTree.envelopeDepth]
        have h' : n = 2 ^ (n + 1).size - 2 := by exact Nat.eq_sub_of_add_eq (id (Eq.symm h))
        let a := (n + 1).size
        have ap : a = value_of% a := by exact rfl
        have ap2 : 2 ^ a = n + 2 := by simp [ap] ; exact h
        simp [←ap] at h'
        have ap3 : (2 ^ a + 1).size = a + 1 := by refine size_add (by grind) (by grind)
        simp [ap2] at ap3
        have ap4 : 2 ^ (n + 2 + 1).size = 2 ^ (a + 1) := by exact congrArg (HPow.hPow 2) ap3
        clear ap3
    ...
    · expose_names
      ...
      · simp [LubyTree.is_envelope, LubyTree.envelopeSize, LubyTree.envelopeDepth] at h
        ...
        have ap3 : 2 ≤ a := by
          ...
          have step2 : (2 + 1).size ≤ (n + 2 + 1).size := by exact Nat.size_le_size (by grind)
        ...
        have ind : n % ((2 ^ (n + 1).size - 1 - 1) / 2) = n + 1 - 2 ^ ((n + 1).size - 1) := by
          ...
          have right : n + 1 - 2 ^ ((n + 1).size - 1) = n + 1 - 2 ^ (n + 1).size / 2 := by
            have : 2 ^ ((n + 1).size - 1) = 2 ^ (n + 1).size / 2 := by
              refine Eq.symm (Nat.div_eq_of_eq_mul_right (by grind) ?_)
              · refine Eq.symm (mul_pow_sub_one ?_ 2)

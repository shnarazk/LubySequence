...
However, outside of Boolean-satisfiability solvers, its applications appear to be rare.
...
To obtain a zero-based version, define \(L(k)\) for \(k\ge0\) by setting \(L(k)=L_1(k+1)\). Equivalently,
...
\begin{align}
	S_2(k) = 2^{(k + 2).size}
\end{align}
where $size : N \longmapsto N $ returns the length of the binary representation of the binary representation of its argument (as in Lean 4).
